...
- **SDK generation:** OpenAPI Generator CLI (PHP, Python, Java); PHP and Java with custom templates in `customtemplates/php/` and `customtemplates/java/`; Python model behavior is generated by `customtemplates/python/`, while `scripts/prepare-python-sdk-layout.py` only cleans packaging files and moves the package under `src/`; Java SDK runtime artifact is a self-contained shaded JAR with dependency relocation.

...
- **SDK generation:** OpenAPI Generator CLI (PHP, Python, Java); PHP and Java with custom templates in `customtemplates/php/` and `customtemplates/java/`; Python model behavior is generated by `customtemplates/python/`, unwanted files are excluded by `--openapi-generator-ignore-list`, while `scripts/prepare-python-sdk-layout.py` removes forced generator metadata and moves the package under `src/`; Java SDK runtime artifact is a self-contained shaded JAR with dependency relocation.
- **Custom schema helper generation:** `x-entity-static-builder` is consumed by PHP/Java/Python templates to generate `createWithMeta(...)` / `create_with_meta(...)`
- **Testing:** PHPUnit (PHP golden), pytest (Python golden), Maven Surefire (Java golden + smoke), Schemathesis (contract)
...
tests/fixtures/                        # Shared golden fixtures for PHP, Python and Java
...
| `generate-sdk`           | `generate-sdk-php`, `generate-sdk-python`, `generate-sdk-java`                                                                                   | push / web / master                                         |
| `test`                   | `sdk-golden-php`, `sdk-golden-python`, `sdk-golden-java`, `sdk-smoke` (java)                                                                     | push / web / master                                         |
...
| `prepare-sdk-repository` | `prep-branch-and-mr-php`, `prep-branch-and-mr-java`, `prep-branch-and-mr-python`, `merge-branch-php`, `merge-branch-java`, `merge-branch-python` | push/web (branch) / master push                             |
...
1. **Push to branch** — lint, bundle, generate PHP/Python/Java SDK, run golden tests and Java-based smoke, then prepare branches in all internal SDK repositories.
2. **Manual (web) on branch** — same as push + contract test flow (`deploy-contract-env` → `create-contract-user` → `sdk-contract`, optional `remove-contract-env`) + optional remote/test package publishing jobs.
3. **Master merge/push** — checks, contract test flow, SDK generation + tests, `version:auto` (CHANGELOG + tag), mirror/release jobs, manual internal SDK release sync for PHP/Java/Python, then package publication jobs.

...
- **SDK generation:** OpenAPI Generator CLI (PHP, Python, Java, TypeScript with custom templates in `customtemplates/php/`, `customtemplates/java/` and `customtemplates/typescript/`); Python model behavior is generated by `customtemplates/python/`, unwanted files are excluded by `--openapi-generator-ignore-list`, while `scripts/prepare-python-sdk-layout.py` removes forced generator metadata and moves the package under `src/`; Java SDK uses Jackson 3 and is published as a self-contained shaded JAR with dependency relocation.
